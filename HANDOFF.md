...
- **Two flagged design deviations** (`0633cc3`): the competitor-detail baseline pill now reads
  "What we're watching now" (matching §03 of `TrailWatch IA Redesign.dc.html`, lines 448/486 — the
  compact "Watching now" at line 645 is only the pricing-history variant); Competitors index cards
  render the bare registrable host (`siteOf`, e.g. `notion.so`, `linear.app`) instead of the full
  `originOf` origin. The card/link still points at the actual tracked page URL.
- **Competitor-detail header host** (`8078582`): same `siteOf` treatment so the header shows
  `notion.so` not `https://www.notion.so`, matching the index + design line 408. `originOf` stays for
  the edit-dialog sibling-domain matching (logic, not display).
- **Deleting the last page deletes the competitor** (`180bd79`): a competitor with zero pages can't
  be tracked, so when the page being deleted is the competitor's only one, the confirm dialog now
  says so explicitly ("<label> is the only page you track for <competitor>, so deleting it removes
  <competitor> from your watchlist too…") and calls `deleteCompetitor` (pages/snapshots/changes/
  insights cascade via FK) instead of `deletePage`. Implemented by threading an `isLastPage` flag
  (page has no sibling) through `dashboard/page.tsx → DashboardTypeCard → DashboardPageRow →
  PageActionsMenu`, and branching the competitor-detail page's own inline confirm on
  `pages.length === 1` (it already routes to `/competitors` after a competitor delete). Multi-page
  competitors keep the plain "Delete this page?" flow. Verified live on both surfaces, single- and
  multi-page; 154 tests pass, typecheck/lint clean, no new migrations.
...
- **Bug #2 — `www`-insensitive dup check — FIXED (`f3ed67c`).** `AddPageDialog`'s local `normalize()`
...
- **Slice 3 — Add Page modal — DONE + design-matched** (this session, verified live incl. a real add
  persisting `page_type`). `AddPageDialog.tsx` was reworked from the old multi-row form into the
  focused single-page modal (IA "Add flows" §2, contextual variant): fixed competitor (avatar + name,
  no picker — it only opens from a competitor's detail page), one **Page URL** field with live
  validation (bare domains normalized to `https://`; format + domain-match via `rowValidation`, plus a
  **duplicate-within-this-competitor** check → "You're already tracking this page"; ✓/⚠ affordance +
  ok/err note), a **Page name** field, and a **Page type** segmented picker (the six `PAGE_TYPE_VALUES`;
  selected = dark pill + lime check; **auto-follows the name via `labelToType` until the user picks**).
  Footer shows `{plan} · will be N of M` + Cancel + **Start tracking** (disabled with reason until a
  valid URL + name). At the per-competitor page cap the form is swapped for a **plan-limit block** —
  Free gets an "Upgrade to Pro" (→ `/billing`) + "Maybe later"; **Pro-at-cap** gets a plan-aware
  "that's the most pages we track… Remove one" + "Got it" (no bogus upgrade CTA). CSS rewritten in
  `AddPageDialog.module.css`. Wiring: `readPageRows` + `addPages` now persist `page_type`; `addPages`
  redirects back to `/competitors/[id]` (the detail) so the new page appears in place; `queries.ts`
  selects `page_type` (0008) with a label-derived fallback for rows still on the `'other'` default.
  `CompetitorDetail` now **always** shows "+ Add page" (the at-limit case lives in the modal, per the
  design — the earlier Slice-2 upgrade-link branch was removed). **Live-unreachable (404) pre-submit is
  intentionally NOT built** — it needs a server round-trip and the design says you can track anyway;
...
- **Slice 5 — Edit competitor + Edit URL — DONE + design-matched** (this session, verified live — §07
  in commit `df7ba3b`, §06 verified with a real rename + page-deletion). Two pieces:
  - **§07 Edit URL** (`components/EditPageDialog.tsx`, reworked): a URL-only correction — Current URL
    shown **locked** (with a lock glyph), New URL validated live (format + same-site, must differ from
    current), Save → `updatePage` re-points + re-checks. The page **name/label is left unchanged**
    (URL-only per the design — the previous dialog also edited the label; that was dropped). Copy never
    claims history carries over (it doesn't — URL change wipes + re-captures, owner's decision). Added
    an **account-wide unique-URL backstop** to `updatePage`. Callers (`CompetitorDetail` kebab,
    `DashboardEditUrl`) unchanged (same props).
  - **§06 Edit competitor** (`competitors/[id]/EditCompetitorDialog.tsx` + `.module.css`): the shared
    add edit view, pre-filled with the competitor + its pages (each row's real `page_type`), in a modal
    opened from the detail **Edit** button (now a `<button>`, not a link). Name + page rows (URL +
    `PageTypeSelect` + remove), "+ Add page" to the cap, soft cross-domain note, account-wide dup +
    same-site validation (shared with the takeover), **the last row can't be removed**, footer
    justify-end (Cancel + "Save changes"). Reuses `CompetitorSetup.module.css` for the field/row styles.
    The old full-page **`/competitors/[id]/edit` route + `EditCompetitorForm` were DELETED** (the dialog
    replaces them).
  - **Shared row rules extracted** to `features/competitors/rowRules.ts` (`toFullUrl`, `canonUrl`,
    `hostname`, `rowUrlError` with the format→dup→domain precedence) — used by BOTH `CompetitorSetup`
    (refactored to import it) and `EditCompetitorDialog`, so add + edit validate identically.
  - **`PageTypeSelect` is now a pure control with NO form `name`** — it never submits on its own;
    callers carry the chosen type in their own hidden `pageType` input (fixing a latent duplicate-field
    risk). **`updateCompetitorDetails` enhanced**: persists `page_type`, **deletes pages removed** on
    the edit screen (present in the DB but not resubmitted), and does the **account-wide dup check**
    (excluding this competitor's own pages).
  - ⚠️ **Gotcha fixed during build:** the Edit dialog's competitor-name input needs a hidden
    `name="name"` input (a nameless visible input doesn't submit → "expected string, received null").
